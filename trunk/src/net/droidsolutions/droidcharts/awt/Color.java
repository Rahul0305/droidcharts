 ...
 * RGB color space. For further information on sRGB, see <A
 ...
 * @see AlphaComposite
 */
public class Color implements Transparency, java.io.Serializable {
	 ...
	 * {@link IllegalArgumentException} if the value is out of
	 * 
	 * @param r
	 *            the Red component
	 * @param g
	 ...
	 * boolean rangeError = false; String badComponentString = ""; int n =
	 * cspace.getNumComponents(); fvalue = new float[n]; for (int i = 0; i < n;
	 * i++) { if (components[i] < 0.0 || components[i] > 1.0) { rangeError =
	 * true; badComponentString = badComponentString + "Component " + i + " "; }
	 * else { fvalue[i] = components[i]; } } if (alpha < 0.0 || alpha > 1.0) {
	 * rangeError = true; badComponentString = badComponentString + "Alpha"; }
	 * else { falpha = alpha; } if (rangeError) { throw new
	 * IllegalArgumentException( "Color parameter outside of expected range: " +
	 * badComponentString); } frgbvalue = cspace.toRGB(fvalue); cs = cspace;
	 * value = ((((int)(falpha*255)) & 0xFF) << 24) |
	 * ((((int)(frgbvalue[0]*255)) & 0xFF) << 16) | ((((int)(frgbvalue[1]*255))
	 * & 0xFF) << 8) | ((((int)(frgbvalue[2]*255)) & 0xFF) << 0); }
	 */
	/**
	...
	public float[] getComponents(ColorSpace cspace, float[] compArray) {
		/*
		 * if (cs == null) { cs = ColorSpace.getInstance(ColorSpace.CS_sRGB); }
		 */
		float f[];
		if (fvalue == null) {
			f = new float[3];
		...
		return compArray;
		/*
		 * float tmp[] = cs.toCIEXYZ(f); float tmpout[] =
		 * cspace.fromCIEXYZ(tmp); if (compArray == null) { compArray = new
		 * float[tmpout.length + 1]; } for (int i = 0 ; i < tmpout.length ; i++)
		 * { compArray[i] = tmpout[i]; } if (fvalue == null) {
		 * compArray[tmpout.length] = ((float)getAlpha())/255f; } else {
		 * compArray[tmpout.length] = falpha; } return compArray;
		 */
	}
	...
	public float[] getColorComponents(ColorSpace cspace, float[] compArray) {
		...
		/*
		 * float tmp[] = cs.toCIEXYZ(f); float tmpout[] =
		 * cspace.fromCIEXYZ(tmp); if (compArray == null) { return tmpout; } for
		 * (int i = 0 ; i < tmpout.length ; i++) { compArray[i] = tmpout[i]; }
		 * return compArray;
		 */
	}
	 ...
	 * r, Rectangle2D r2d, AffineTransform xform, RenderingHints hints) { if
	 * (context == null || context.getRGB() != getRGB()) { context = new
	 * ColorPaintContext(getRGB(), cm); } return context; }
	 */
	/**
