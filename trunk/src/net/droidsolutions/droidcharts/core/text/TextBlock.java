 ...
 * Contributor(s):   -;
 *
 * $Id: TextBlock.java,v 1.14 2004/05/19 08:18:08 mungady Exp $
 *
 * Changes
 * -------
...
import net.droidsolutions.droidcharts.common.ShapeUtils;
...
public class TextBlock implements Serializable {
	 ...
	 * @return the bounds.
	 */
	public Shape calculateBounds(final Canvas g2, final float anchorX,
		...
		final float[] offsets = calculateOffsets(anchor, d.getWidth(), d
				.getHeight());
		final Rectangle2D bounds = new Rectangle2D.Double(anchorX + offsets[0],
				anchorY + offsets[1], d.getWidth(), d.getHeight());
		final Shape rotatedBounds = ShapeUtils.rotateShape(bounds, angle,
				rotateX, rotateY);
	...
	public void draw(final Canvas g2, final float x, final float y,
			final TextBlockAnchor anchor, Paint paint) {
		draw(g2, x, y, anchor, 0.0f, 0.0f, 0.0, paint);
	}
	 ...
	 */
	/*
	 * public void draw(final Canvas g2, final float anchorX, final float
	 * anchorY, final TextBlockAnchor anchor, final float rotateX, final float
	 * rotateY, final double angle) {
	 * 
	 * final Size2D d = calculateDimensions(g2); final float[] offsets =
	 ...
	 * line.draw(g2, anchorX + offsets[0], anchorY + offsets[1] + yCursor,
	 * TextAnchor.TOP_LEFT, rotateX, rotateY, angle); final Size2D dimension =
	 * line.calculateDimensions(g2); yCursor = yCursor + (float)
	 * dimension.getHeight(); }
	 * 
	 * }
	 */
	...
	public void draw(final Canvas g2, final float anchorX, final float anchorY,
			...
			final float rotateY, final double angle, Paint paint) {
