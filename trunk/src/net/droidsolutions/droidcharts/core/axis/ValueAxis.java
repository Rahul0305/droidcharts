...
public abstract class ValueAxis extends Axis implements Cloneable, Serializable {
	...
	protected void drawAxisLine(Canvas g2, double cursor, Rectangle2D dataArea,
		...
		Paint p = getAxisLinePaint();
		p.setStyle(Paint.Style.STROKE);
		p.setStrokeWidth(getAxisLineStroke());

		g2.drawLine((float) axisLine.getX1(), (float) axisLine.getY1(),
				(float) axisLine.getX2(), (float) axisLine.getY2(), p);
	 ...
	 */
	protected AxisState drawTickMarksAndLabels(Canvas g2, double cursor,
		...
		while (iterator.hasNext()) {
			...
			if ((isTickMarksVisible() && tick.getTickType().equals(
				...
				float xx = (float) valueToJava2D(tick.getValue(), dataArea,
						edge);
				Line2D mark = null;
				float tickStroke = getTickMarkStroke();
				Paint tickMark = getTickMarkPaint();
				tickMark.setStrokeWidth(tickStroke);
				...
				g2.drawLine((float) mark.getX1(), (float) mark.getY1(),
	 ...
	 */
	protected double findMaximumTickLabelHeight(List ticks, Canvas g2,
			Rectangle2D drawArea, boolean vertical) {

		RectangleInsets insets = getTickLabelInsets();
		// Font font = getTickLabelFont();
		double maxHeight = 0.0;
		Paint paint = getTickLabelPaint();
		if (vertical) {
			...
			Rectangle2D labelBounds = TextUtilities.getTextBounds("ABCxyz",
					paint);
	 ...
	 */
	protected double findMaximumTickLabelWidth(List ticks, Canvas g2,
			Rectangle2D drawArea, boolean vertical) {
	...
	public void resizeRange(double percent, double anchorValue) {
		if (percent > 0.0) {
			...
			Range adjusted = new Range(anchorValue - halfLength, anchorValue
					+ halfLength);
			setRange(adjusted);
		} else {
			setAutoRange(true);
